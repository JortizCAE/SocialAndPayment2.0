...
export default function SelectField(props: SelectFieldProps) {
  ...
  return (
    <div className={title ? 'px-6 py-5 mb-2' : ''}>
      {title && <h4 className="font-medium text-30xl mb-4 text-center lg:text-left">{title}</h4>}

      <Carousel responsive={responsive}>
        {items.map(({ id, name, describe, thumb }, index) => {
          const getVrnt = vrnt[variant][index === value || readonly ? 'select' : 'unselect'];
            ...
            <div className={readonly ? 'mx-2' : 'mx-6'}>
              ...
              >
                <div className="flex flex-col p-2.5 min-w-[63px]">
                  <h5 className={`text-black font-bold font-montserrat ${getVrnt.title}`}>
                    {name}
                  </h5>
                  <p className={`font-roboto text-gray-200 ${getVrnt.describe}`}>{describe}</p>
                </div>
                <div
